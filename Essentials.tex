...
\begin{compactdesc}
    ...
    \item[Proj. Matrix:] $U = \left[\mathbf{u_1}\vert ... \vert \mathbf{u_n}\right], PX=UU^\top X, det(P)\in\{0, 1\}$
    \item[Rank:] $rank(A)=dim(im(A)) \leq min\{m,n\},$ $rank(AB) \leq min\{rank(A), rank(B)\}$
	...
	\begin{inparaitem}
		...
		\item $\langle \mathbf{x}, \mathbf{y} \rangle = \|\mathbf{x}\|_2 \cdot \|\mathbf{y}\|_2 \cdot \cos(\theta) \rightarrow \|\langle \mathbf{x}, \mathbf{y} \rangle\|^2 \leq \|\mathbf{x}\|_2^2 \cdot \|\mathbf{y}\|_2^2$ (CS IE.)
...
$\frac{\partial a^\top X^{-1} b}{\partial X} = -X^{-T}ab^\top X^{-T}$\\
...
$\frac{\partial}{\partial \mathbf{X}}(\|\mathbf{X}\|_F^2) = 2\mathbf{X}$, $\frac{\partial det(A)}{\partial A} = det(A)(A^{-1})^\top$ \\
...
if $\mathbf{A}$ symm. iff $\exists \mathbf{Q}$ orthogonal s.t. $A = \mathbf{Q} \boldsymbol{\Lambda} \mathbf{Q^\top}$
...
\end{inparaitem}
\begin{compactdesc}
    \item $var(\mathbf{x})=\mathbb{E}[\mathbf{x}\mathbf{x}^\top]-\mu\mu^\top$
    \item $cov(\mathbf{x}, \mathbf{y}) = \mathbb{E}[(\mathbf{x}-\mu_y)(\mathbf{y}-\mu_x)]$
\end{compactdesc}
